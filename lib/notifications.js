...
// ── Rappel quotidien (notification locale, natif uniquement) ─────────────────
...
// Anciennement "rappel flammes" (streak 1-à-1) — reformulé pour les Compétitions.
const DAILY_REMINDER_ID = 'ootd-daily-reminder';
...
export async function scheduleDailyReminder(hour = 19, minute = 0) {
  ...
  try {
    ...
    await Notifications.scheduleNotificationAsync({
      ...
      content: {
        title: 'OOTD',
        body: "C'est l'heure de prendre ta photo du jour pour tes compétitions ! 🏆",
        data: { url: '/?analyse=1' },
      },
      trigger: { hour, minute, repeats: true },
    });
  } catch (e) {
    console.log('[notifications] scheduleDailyReminder', e?.message || e);
  }
}
