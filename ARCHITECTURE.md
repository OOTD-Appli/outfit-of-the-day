...
Reprend le contenu de l'ancien `ProfilScreen` quasiment tel quel (modal Réglages, galerie paginée + lightbox, carte Niveau — lift-and-shift, pas une réécriture), réorganisé. **Ordre revu le 2026-09-25** (refonte purement structurelle/visuelle, aucune nouvelle donnée ni requête — la carte "Niveau" et la logique de tous les points ci-dessous étaient déjà là avant, seul l'agencement change) :

1. En-tête : titre "Récap" + bouton "Télécharger l'app" (PWA, affiché seulement si `installable`) — le bouton déconnexion n'y est plus, voir carte "Compte" plus bas
2. Carte profil : avatar, pseudo, badge abonnement, stats (tenues / score moyen / points), top 3 styles
3. **Graphique perso "Mon évolution"** (2026-09-25, décision D6 — remplace les 2 blocs Top 3, déménagés dans `PalmaresScreen`) : toggle 7/30 jours, ligne SVG (`Polyline`+`Circle`, `react-native-svg`) tracée à partir du state `ootds` déjà chargé par `fetchProfil` (aucune requête réseau dédiée — peut donc ne pas couvrir toute la fenêtre si l'utilisateur publie plus de 21 fois sur la période, accepté pour cette V1), score normalisé via `(score_global / score_scale) * 100` (même formule que `moyenneScore`) — **remonté juste après la carte profil** (2026-09-25, était tout en bas de l'écran avant)
4. **Carte "Compte"** (nouveau, 2026-09-25 — remplace les 3 anciens boutons Réglages/Abonnement/Mes amis + le bouton déconnexion du header) : 4 lignes empilées avec séparateurs fins — Réglages et Mes amis (icône neutre, chevron, simples liens de navigation) ; Abonnement (icône dorée `#FFC94D` + sous-texte + badge du plan actuel Free/Plus/Elite au lieu du chevron, traité différemment car c'est une action commerciale) ; Se déconnecter (icône + libellé en rouge `#FF5C7A`, aucun élément de fin, action terminale). Ces deux couleurs sont des accents sémantiques fixes (constantes `GOLD`/`DANGER` dans le fichier), indépendants du thème cosmétique choisi par l'utilisateur — contrairement à `theme.accent` utilisé partout ailleurs sur cet écran.
5. Galerie "Mes tenues" — grille 3 colonnes **avec bordure et espacement entre chaque photo** (`gridCell` padding 3 + `gridPhoto` bordure `rgba(128,128,128,0.28)`), pagination 21/page (tier Gratuit plafonné à la 1ère page), lightbox horizontal swipe enrichie (badge note globale **dénominateur dynamique** `/{score_scale}` — voir "Note sur 100" plus bas —, 3 badges colorés, chips de style, description, conseils IA structurés). **Ajout (2026-09-25)** : badge de score en médaillon (coin haut-droit) directement sur chaque vignette de la grille (même normalisation `/100` que `moyenneScore`), en plus du badge déjà présent dans la lightbox plein écran.
6. Carte "Niveau" (`computeLevelInfo`) — **descendue tout en bas de l'écran** (2026-09-25, était juste après la carte profil avant), rendue dans le `ListFooterComponent` de la `FlatList` (après la galerie), contenu inchangé
...
Ligne "Abonnement" de la carte Compte → `navigation.navigate('Shop')` (même stack, `RecapStack`). Ligne "Mes amis" → `navigation.navigate('Friends')`.
